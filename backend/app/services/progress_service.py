...
from app.models.progress import UserXP, Streak, Badge, UserBadge, UserProgress
...
from app.schemas.progress import (
    ...
    DayActivity, RecommendedLesson, BudgetSnapshot, SIPSnapshot
)
...
def compute_streak(last_activity: date | None, current: int) -> tuple[int, bool]:
    today = date.today()
...
async def check_badges(db: AsyncSession, user_id: uuid.UUID, xp_row: UserXP, streak_row: Streak) -> list[str]:
    """Award any newly earned badges. Returns list of new badge names."""
    ...
    # Completed modules
    modules_done_result = await db.execute(
        select(func.count(func.distinct(Lesson.module_id)))
        .join(UserProgress, (UserProgress.lesson_id == Lesson.id) & (UserProgress.user_id == user_id) & (UserProgress.completed == True))
    )
    modules_done = modules_done_result.scalar() or 0
...
async def get_dashboard(db: AsyncSession, user_id: uuid.UUID) -> DashboardResponse:
    ...
    long_streak = streak_row.longest_streak if streak_row else 0

    # Lesson counts
    total_lessons = await db.scalar(select(func.count(Lesson.id)).where(Lesson.is_published == True)) or 0
    completed_lessons = await db.scalar(
        select(func.count(UserProgress.id)).where(
            UserProgress.user_id == user_id, UserProgress.completed == True
        )
    ) or 0
    completion_pct = round(completed_lessons / total_lessons * 100, 1) if total_lessons else 0.0
    ...
    for i in range(6, -1, -1):
        ...
        xp_day = await db.scalar(
            select(func.coalesce(func.sum(UserProgress.xp_earned), 0)).where(
                UserProgress.user_id == user_id,
                UserProgress.completed == True,
                UserProgress.completed_at >= day_start,
                UserProgress.completed_at <= day_end,
        ...
        weekly_activity.append(DayActivity(date=day.isoformat(), lessons_completed=count, xp_earned=xp_day))
    ...
    for ub in recent_user_badges:
        badge_result = await db.scalar(select(Badge).where(Badge.id == ub.badge_id))
        if badge_result:
    ...
    # Recommended: next unfinished lesson per track
    ...
    for track in ["A", "B", "C"]:
        modules_result = await db.execute(
            select(Module).where(Module.track == track, Module.is_published == True)
            .order_by(Module.order_index)
        )
        for mod in modules_result.scalars():
            lessons_result = await db.execute(
                select(Lesson).where(Lesson.module_id == mod.id, Lesson.is_published == True)
                .order_by(Lesson.order_index)
            )
            for lesson in lessons_result.scalars():
                if lesson.id not in completed_ids:
                    recommended.append(RecommendedLesson(
                        lesson_id=lesson.id,
    ...
    plan_result = await db.scalar(
        select(BudgetPlan).where(BudgetPlan.user_id == user_id, BudgetPlan.month == today_first)
    )
    if plan_result:
        entries_result = await db.execute(
            select(BudgetEntry).where(BudgetEntry.plan_id == plan_result.id)
        )
        entries = entries_result.scalars().all()
        total_actual = sum(float(e.actual) for e in entries)
        savings = sum(float(e.actual) for e in entries if e.category in ("savings", "investments"))
    ...
    calc_result = await db.execute(
        select(CalculatorHistory).where(CalculatorHistory.user_id == user_id)
        .order_by(CalculatorHistory.created_at.desc()).limit(3)
    )
    calc_history = [SIPSnapshot.model_validate(c) for c in calc_result.scalars()]
